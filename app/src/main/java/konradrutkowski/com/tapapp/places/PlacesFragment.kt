...
class PlacesFragment : Fragment(), SwipeRefreshLayout.OnRefreshListener {
    ...
    private fun initSwipe() {
        swipe_layout.run {
            setOnRefreshListener(this@PlacesFragment)
    ...
    override fun onRefresh() {
        if (locationTracker.canGetLocation() && NetworkConnection.isInternetReady(this.activity)) {
            val dataCollector = DataCollector(this, swipe_layout, RequestConfig.latitude, RequestConfig.longtitude)// FAKELOC
    ...
    companion object {
        const val FOURSQUARE_OBJECT_KEY = "FOURSQUARE_OBJECT"
